...
This system is not intended to less expensive than other options out in the aftermarket stores. Depending on where your buying the parts it would likely be around 80-100 dollars in parts assuming you have some of the essentials like screws and heat set inserts. With all the hardware testing I did it was notably more expensive than an existing product. However, now that it is done, I have exactly what I wanted, it is configurable and updatable in ways store bought options could never be. This has been a passion project that is now ready to be tested on my Masterbuilt Gravity smoker.
...
- PWM fan control with RPM feedback
  - it is worth noting that the system does not have to drive a fan. With some minor changes this system could drive a servo instead of a fan to control an ball valve. This would allow you to control passive fire systems like other Gravity Smokers.
